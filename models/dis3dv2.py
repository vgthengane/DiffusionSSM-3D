...
from DiS.mamba.mamba_ssm.modules.mamba2_simple import Mamba2Simple 
from DiS.mamba.mamba_ssm.ops.triton.layernorm import RMSNorm, layer_norm_fn, rms_norm_fn
...
def unpatchify_voxels(x0, input_size, patch_size, channels=3):
    """
...
def create_block(
    ...
    **kwargs
    ...
    mixer_cls = partial(Mamba2Simple, layer_idx=layer_idx, 
                        # bimamba_type=bimamba_type, 
                        **ssm_cfg, **factory_kwargs, **kwargs)
    norm_cls = partial(
...
class DisModel(nn.Module): 
    def __init__(
        ...
        **kwargs
        ...
        self.patch_size = patch_size
        self.voxelization = Voxelization(resolution=img_size, normalize=True, eps=0)
        self.patch_embed = PatchEmbed_Voxel(patch_size=patch_size, in_chans=channels, embed_dim=embed_dim)
    ...
    def forward(self, x, timesteps, labels=None, inference_params=None): 
        ...
        x = self.patch_embed(x) # patch_embed_voxel
        B, L, D = x.shape
        ...
        x = x[:, self.extras:, :]
        x = unpatchify_voxels(x, self.input_size, self.patch_size, self.channels
                            #   self.out_channels
                            )
