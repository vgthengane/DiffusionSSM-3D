...
def get_dataset(dataroot, npoints, category, use_mask=False, 
                ntrain_samples=None, nval_samples=None):
    tr_dataset = ShapeNet15kPointClouds(root_dir=dataroot,
        ...
        random_subsample=True, use_mask = use_mask, num_samples=ntrain_samples)
    te_dataset = ShapeNet15kPointClouds(root_dir=dataroot,
        ...
        use_mask=use_mask, num_samples=nval_samples
    )
    return tr_dataset, te_dataset


def evaluate_gen(opt, ref_pcs, logger, batch_size):

    if ref_pcs is None:
        _, test_dataset = get_dataset(opt.dataroot, opt.npoints, opt.category, use_mask=False)
        test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size if batch_size is not None else opt.batch_size,
                                                      shuffle=False, num_workers=int(opt.workers), drop_last=False)
        ref = []
        for data in tqdm(test_dataloader, total=len(test_dataloader), desc='Computing Reference Samples'):
            x = data['test_points']
        ...
        logger.info("Loading reference point path: %s"
            % (opt.eval_path))
        sample_pcs = torch.load(opt.ref_pcs_path).contiguous()
    ...
    # Compute metrics
    results = compute_all_metrics(sample_pcs, ref_pcs, batch_size if batch_size is not None else opt.batch_size)
...
def generate(model, opt, batch_size=None, epoch=None):

    kwargs = {
        "ntrain_samples": opt.ntrain_samples, 
        "nval_samples": opt.nval_samples
    } if opt.small_exp else {}
    _, test_dataset = get_dataset(opt.dataroot, opt.npoints, opt.category, **kwargs)
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm(enumerate(test_dataloader), total=len(test_dataloader), desc='Generating Samples'):
            ...
            visualize_pointcloud_batch(os.path.join(str(Path(opt.eval_path).parent), f'eval_e{epoch:04d}_b{i:02d}.png'), gen[:64], None,
                                       None, None)
...
def main(opt):
    ...
    eval_id = opt.model.split("/")[-3].replace("training", "evaluation")  
    output_dir = os.path.join(opt.model_dir, opt.experiment_name, eval_id)
    ...
    with torch.no_grad():
        ...
        logger.info("Evaluation model Path:%s" % opt.model)
